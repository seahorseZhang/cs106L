...
bool operator==(const HashMap<K, M, H>& lhs, const HashMap<K, M, H>& rhs) {
    ...
    for (auto iter = lhs.begin(); iter != lhs.end(); ++iter) {
        typename HashMap<K, M, H>::const_iterator finder = rhs.find(iter->first);
        if (finder == rhs.end() || finder->second != iter->second) {
            return false;
        }
    }
    return true;
...
HashMap<K, M, H>::HashMap(const HashMap& map) :
    _size(0),
    _hash_function(map._hash_function), _buckets_array(map.bucket_count(), nullptr) {
    auto iter = map.begin();
    while (iter != map.end()) {
        (void)insert(*iter);
        ++iter;
...
HashMap<K, M, H>::HashMap(HashMap&& map) :
    ...
    _hash_function(std::move(map._hash_function)), _buckets_array(std::move(map._buckets_array)) {
    map._size = 0;
}
